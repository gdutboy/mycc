...
- [x] Correction: 记录 findings 时只写索引不写详情 — 索引必须配合详情段落，否则后续 session 无法还原上下文 #finding [date:: 2026-03-11]
...
- [-] 外部计划源通用化架构 — 已排除：当前只有 1 个上游工具，过早抽象不值得 #finding [date:: 2026-02-17]
...
- [-] OpenSpec 对比分析 — 已排除：Artifact 结构差异大（变更文件夹 vs 根文件+双区），无紧迫需求 #finding [date:: 2026-02-16]
...
**状态说明**: `[x]` 已采纳/已验证 | `[-]` 已排除/已否定 | `[ ]` 参考中/待评估

## 未解决问题

- [ ] Hermes Agent 竞品调研 — Nous Research 自改进 AI 代理，技能自改进机制 + 多终端后端 + cron 调度 #finding [date:: 2026-03-12]

### [2026-03-12] Hermes Agent 竞品调研

**背景**：用户要求查看 https://github.com/NousResearch/hermes-agent，寻找可借鉴点。

**项目定位**：
- **唯一具有内置学习循环的代理**：从经验创建技能、使用时自我改进、主动持久化知识、搜索过去对话、跨会话建立用户模型
- **多平台部署**：支持 $5 VPS、GPU 集群、serverless 基础设施

**技术栈**：Python 87% + Node.js，uv 包管理

**核心特性**：
| 特性 | 实现 |
|------|------|
| 学习循环 | 代理自主记忆 + 周期性提醒 + 技能自改进 |
| 子代理并行 | 隔离子代理生成 + RPC 调用工具 |
| 记忆系统 | FTS5 会话搜索 + LLM 摘要 + Honcho 用户建模 |
| 技能系统 | 程序化记忆 + 技能中心 + 动态创建 |
| 多终端后端 | local、Docker、SSH、Daytona、Singularity、Modal |
| Cron 调度 | 自然语言描述定时任务，投递到任意平台 |

**与 MyCC 相关性**：
- 无 Claude Code 直接集成
- 技能自改进机制值得参考（mycc 目前 skill 是静态的）
- 多终端后端架构可借鉴（mycc 当前仅飞书 + 网页）
- cron 调度是 mycc 缺失的能力

**可借鉴方向**：
1. **技能动态更新**：当前 mycc skill 是静态定义，可探索运行时更新能力
2. **定时任务增强**：scheduler 目前基于 cron，可增加自然语言描述能力
3. **用户建模**：hermes 的跨会话用户模型值得参考

**行动建议**：
- 短期：无直接行动
- 中期：scheduler skill 可考虑自然语言定时描述
- 长期：探索 skill 动态更新可能性
...
### [2026-03-12] 已归档：Correction 记录 findings 时只写索引不写详情（2026-03-11）
→ 索引见第8行 `[x] Correction: 记录 findings 时只写索引不写详情`，已标记 [x] 且已复盘

### [2026-03-09] S-1 统一 stdin 解析方向（已归档）
→ 索引见第16行 `[x] S-1 统一 stdin 解析方向`
...
### [2026-03-12] 已归档：knowledge 注入仅 L0 摘要（2026-03-08）
→ 索引见第9行 `[x] knowledge 注入仅 L0 摘要`

### [2026-03-12] 已归档：AI 记录 findings 习惯性跳过详情（2026-03-08）
→ 索引见第9行 `[x] AI 记录 findings 习惯性跳过详情`，已标记 [x] 且有对应 CHG
→ 详情已归档到 Claude Code 知识库，跨 session 可通过 knowledge/ai-verification-discipline.md 查阅

### [2026-03-12] 已归档：指引体系补遗（2026-03-08）
→ 索引见第11行 `[x] 指引体系补遗`，已完成 CHG-20260308-04
