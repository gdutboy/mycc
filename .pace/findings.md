...
- [ ] PACEflow v4.8.1 设计全面评估 — 6 维度评分：设计哲学 A/架构健壮性 A-/协议完整性 B+/测试覆盖 B/可维护性 A-/实际效果 A；最大风险 install.js 零测试；核心差距 E 阶段中途纠偏+无回滚机制；25+ CHG 迭代验证核心设计决策正确 #finding [date:: 2026-03-07]
...
**背景**：Antigravity 审查报告（S-1 建议）指出 8 个 hook 脚本各自独立解析 stdin JSON，存在 ~40 行重复代码。

**当前状态**：每个 hook 用类似模式解析 stdin：
```javascript
let toolName = '', fileName = '';
try {
  const input = JSON.parse(fs.readFileSync(0, 'utf8'));
  toolName = input.tool_name || '';
  fileName = (input.tool_input?.file_path || '').replace(/\\/g, '/');
} catch(e) {}
```
...
| Hook | 需要的字段 |
|------|-----------|
| pre-tool-use | tool_name, tool_input.file_path, tool_input.content, tool_input.old_string, tool_input.new_string |
| post-tool-use | tool_name, tool_input.file_path, tool_input.new_string |
| todowrite-sync | tool_name, tool_input |
| stop | type, last_assistant_message |
| session-start | type |
| pre-compact | （无特定字段） |
| config-guard | tool_input |

**可行方案**：pace-utils.js 新增 `parseHookStdin()` 函数，返回统一结构 `{ toolName, fileName, content, oldString, newString, type, lastMessage, raw }`。各 hook 解构取所需字段。

**收益**：~40 行代码减少，错误处理统一（当前部分 hook 的 catch 为空，部分有 fallback），stdin 字段命名一致化。

**风险**：低。纯重构，不改变行为。但需注意 session-start 读 stdin 的时机（L28-32）与其他 hook 不同（顶层 vs 条件分支内）。

**优先级**：P3（代码质量改进，非功能性）。

### [2026-03-09] S-3 版本自动化方向

**背景**：Antigravity 审查报告（S-3 建议）指出版本号分散在 5+ 个文件，当前靠手动同步 + verify.js 被动检查。每次版本变更需编辑 pace-utils.js + plugin.json + marketplace.json + REFERENCE.md + README.md。

**当前版本管理**：
- 权威源：`pace-utils.js` 的 `PACE_VERSION` 常量
- 需同步：`plugin.json:version`、`marketplace.json:plugins[0].version`、`REFERENCE.md:L1,3`、`README.md:L3`
- 检测：`verify.js` 组 7 比较 plugin.json vs PACE_VERSION

**可行方案**：创建 `paceflow/bump-version.js` 脚本：
```bash
node paceflow/bump-version.js 5.0.3
# 自动更新 5 个文件的版本号 + 输出 diff 摘要
```

**实现要点**：
1. 读取当前 PACE_VERSION → 正则替换到 5 个目标文件
2. plugin.json/marketplace.json 用 JSON.parse→修改→JSON.stringify
3. REFERENCE.md/README.md 用正则替换版本号模式
4. 输出变更摘要供用户确认

**收益**：消除手动同步遗漏（ticket17 审查中版本不一致是 P0 修复项），版本发布从 5 次 Edit → 1 条命令。

**风险**：低。独立工具脚本，不影响运行时。

**优先级**：P2（提升发布效率，减少人为错误）。

### [2026-03-09] S-4 walkthrough/findings 活跃区无归档机制

**背景**：compact 后 token 消耗 90K+，最大来源是 SessionStart 注入的 walkthrough.md（50+ 行索引表 + 20+ 详情段落）和 findings.md（40+ 索引 + 10+ 详情段落）活跃区全文。

**问题**：5 个 artifact 文件中，task.md 和 implementation_plan.md 有完整的归档保障链：

| 文件 | PostToolUse 提醒 | Stop 阻止 | 效果 |
|------|-----------------|----------|------|
| task.md | H3 warnOnce | exit 2（未归档） | 强制归档 |
| implementation_plan.md | H10 warnOnce | exit 2（[/] 不一致） | 半强制 |
| walkthrough.md | 无 | 无 | **活跃区无限增长** |
| findings.md | 无 | 无 | **活跃区无限增长** |

walkthrough 和 findings 都有 `<!-- ARCHIVE -->` 标记，但无 hook 触发"将已解决/旧条目移到 ARCHIVE 下方"。

**影响量化**：
- walkthrough 活跃区当前 ~300 行（50+ CHG 的索引表 + 20+ 详情段落），每个 CHG 增加 ~15 行
- findings 活跃区当前 ~400 行（40+ 索引 + 10+ 详情段落），详情段落 30-80 行/个
- SessionStart 每次全量注入活跃区，每次 startup/compact 消耗 ~700 行 token

**可行方案**：

1. **PostToolUse 归档提醒**（最小改动，~20 行）：
   - walkthrough：编辑 walkthrough.md 后检查活跃区详情段落数 > N（如 10），提醒归档旧的
   - findings：编辑 findings.md 后检查 `[x]`/`[-]` 索引 + 对应详情在活跃区，提醒归档

2. **Stop 归档检查**（~15 行）：
   - walkthrough 活跃区详情段落数 > N → warning
   - findings `[x]`/`[-]` 索引数 > M 且在活跃区 → warning

3. **SessionStart 智能截断**（不修改文件，~10 行）：
   - walkthrough 注入时只取最近 N 个详情段落（如 5 个）
   - findings 注入时 `[x]`/`[-]` 的详情段落不注入

**推荐**：方案 1+3 组合。方案 1 长期保持活跃区干净，方案 3 短期缓解 token 膨胀。

**优先级**：P1（直接影响每次会话的 token 预算和 compact 频率）。

### [2026-03-07] PACEflow v4.8.1 设计全面评估
...
| 维度 | 评分 | 说明 |
|------|------|------|
| 设计哲学 | **A** | "确定性 hooks + 建议性 CLAUDE.md"分层清晰，避免了纯 LLM 指令的不可靠性 |
| 架构健壮性 | **A-** | pace-utils 公共模块 + 顶层 try-catch + 降级放行，单点故障风险低 |
| 协议完整性 | **B+** | P-A-C-E-V 五阶段覆盖完整，但 E 阶段缺乏中途纠偏机制 |
| 测试覆盖 | **B** | 单元 36 + E2E 48 + verify 6 覆盖 hooks/utils，但 install.js 零测试 |
| 可维护性 | **A-** | 常量集中 + 版本单一源 + 双模板同步，代码风格一致 |
| 实际效果 | **A** | 15 轮审查（ticket1-15）验证核心逻辑健壮，确认问题集中在文档/风格层 |

**核心优势**：
1. **确定性保障层**：hooks 提供 100% 阻止能力（deny/exit 2），不依赖 AI 自律
2. **多信号激活**：`isPaceProject()` 五级判定避免误激活/漏激活
3. **优雅降级**：teammate 场景 deny→hint，异常时 exit 0 静默放行
4. **Vault 统一存储**：`getArtifactDir()` 唯一路径解析器，消除 artifact 分裂

**关键差距**：
1. **install.js 零测试覆盖**（最大风险）：安装脚本处理 settings.json 合并、文件备份、模板清理，任何 bug 直接影响生产环境，但无自动化测试
2. **E 阶段中途纠偏缺失**：执行阶段发现方案有误时，无标准化的"暂停→修正→恢复"流程，依赖 AI 自行判断
3. **无回滚机制**：artifact 归档后无法撤销，依赖 git 版本控制
4. **verify.js 覆盖有限**：健康检查 6 组仅覆盖静态一致性，不覆盖运行时行为

**设计决策验证（经 25+ CHG 迭代确认正确）**：
- `countByStatus().done = [x]+[-]` vs `stop.js xCount = [x]` — 有意区分，不是 bug
- Hook I/O 协议 `exit 0 + stderr = 忽略` — 多次踩坑后成为硬性约束
- `.pace/` 运行时状态 vs Vault artifact 存储 — 关注点分离
- `CLAUDE_PROJECT_DIR` 替代 `.pace/` 向上搜索 — 根因修复 vs workaround

**建议优先级**：
- **P0**：install.js 测试（至少 settings.json 合并 + 文件备份的关键路径）
- **P1**：E 阶段纠偏协议（`[!]` 阻塞状态 → 方案修正 → 恢复执行）
- **P2**：Plugin 化打包（消除手动 install.js 部署，走 CC 原生分发）

## Corrections 记录

### Correction: 新 hook 事件语义未实测确认就实施
...
### Correction: thinking 发现异常但未验证就跳过
...
### Correction: 记录 findings 时只写索引不写详情
