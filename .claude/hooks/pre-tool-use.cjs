...
let paceUtils;
try { paceUtils = require('./pace-utils.cjs'); } catch(e) {
...
paceUtils.withStdinParsed((stdin) => {
  ...
  // 排除 .pace/ 目录（PACE 运行时状态文件不是 artifact）
  if ((toolName === 'Write' || toolName === 'Edit') && artDir !== cwd && paceSignal) {
    if (ARTIFACT_FILES.includes(fileName) && normalizedFile.startsWith(cwdWithSlash) && !normalizedFile.includes('/.pace/')) {
